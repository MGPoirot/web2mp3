...
def manual_track_tags(market='NL') -> pd.Series:
    ...
    found_artist = spotify.search(q=tag_series.artist, market=market, limit=1, type='artist')['artists']['items'][0]
    is_artist = input(f'Is this the artist you were looking for? "{found_artist["name"]}" [Yes]/No') or 'Yes'
...
def set_file_tags(mp3_tags: dict, file_name: str, audio_source_url=None, logger=print):
...
def download_cover_img(cover_img_path: str, cover_img_url: str, logger=print):
    ...
    if res.status_code == 200:
        ...
        raise ConnectionError('Image Couldn\'t be retrieved')
